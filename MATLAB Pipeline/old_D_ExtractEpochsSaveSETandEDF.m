...
outRootDir = 'E:\Dataset\EEG+Pupillometry\DS003838-PRE-CRD-ICA-EPH';
...
for i = 98:-1:32
    ...
    try
	    memoryEEG = pop_epoch( EEG, {  memory{:} }, [-1.5  3.5], 'newname', 'memory_ICA', 'epochinfo', 'yes');
    catch ERR
    ...
    try
	    controlEEG = pop_epoch( EEG, {  control{:} }, [-1.5  3.5], 'newname', 'control_ICA', 'epochinfo', 'yes');
    catch ERR
        diary('pipeline_logfile.txt');
        disp('==================================');
	...
	outEEGFile = strcat(subjectDir, '_memory_PRE-CRD-ICA-EPH_eeg.set');
	...
	outEEGFile = strcat(subjectDir, '_control_PRE-CRD-ICA-EPH_eeg.set');
	outFileDir = fullfile(outRootDir, subjectDir, outEEGFile);
	...
	outEEGFile = strcat(subjectDir, '_memory_PRE-CRD-ICA-EPH_eeg.edf');
	outFileDir = fullfile(outRootDir, subjectDir, outEEGFile);
	pop_writeeeg(memoryEEG, outFileDir, 'TYPE','EDF');
	disp(['memory condition .edf data saved to: ' outFileDir]);


	outEEGFile = strcat(subjectDir, '_control_PRE-CRD-ICA-EPH_eeg.edf');
	outFileDir = fullfile(outRootDir, subjectDir, outEEGFile);
